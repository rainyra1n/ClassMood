...
class HeadPoseService:
    ...
    def video_interest(self, path, frame_skip=5, visualise=False):
            ...
            if frame_count % frame_skip == 0:
                try:
                    ...
                    if predicted:
                        ...
                        time_keys.append(round(frame_count / fps, 3))
        ...
        smoothed_values = self.median_exponential_smoothing(score_values)
        interest_per_time = dict(zip(time_keys, smoothed_values))

        if visualise:
            import matplotlib.pyplot as plt
            x_values = interest_per_time.keys()
            y_values = interest_per_time.values()
    ...
    def median_exponential_smoothing(self, values, window=7, alpha=0.1):
        ...
        for value in median_smoothed[1:]:
            ...
            final_smoothed.append(min(max(s, 0), 100))
...
result = headpose_service.video_interest('videos/1.MOV', frame_skip=10, visualise=True)
